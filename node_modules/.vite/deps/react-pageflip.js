...
//#endregion
//#region node_modules/react-pageflip/build/index.es.js
var import_react = /* @__PURE__ */ __toESM(require_react());
var import_page_flip_browser = require_page_flip_browser();
var HTMLFlipBookForward = import_react.forwardRef((props, ref) => {
	...
	(0, import_react.useImperativeHandle)(ref, () => ({ pageFlip: () => pageFlip.current }));
	const refreshOnPageDelete = (0, import_react.useCallback)(() => {
		if (pageFlip.current) pageFlip.current.clear();
	}, []);
	...
	(0, import_react.useEffect)(() => {
		childRef.current = [];
		if (props.children) {
			const childList = import_react.Children.map(props.children, (child) => {
				return import_react.cloneElement(child, { ref: (dom) => {
	...
	(0, import_react.useEffect)(() => {
		const setHandlers = () => {
			...
			if (flip) {
				...
				if (props.onChangeOrientation) flip.on("changeOrientation", (e) => props.onChangeOrientation(e));
				if (props.onChangeState) flip.on("changeState", (e) => props.onChangeState(e));
		...
		if (pages.length > 0 && childRef.current.length > 0) {
			removeHandlers();
			if (htmlElementRef.current && !pageFlip.current) pageFlip.current = new import_page_flip_browser.PageFlip(htmlElementRef.current, props);
			if (!pageFlip.current.getFlipController()) pageFlip.current.loadFromHTML(childRef.current);
			else pageFlip.current.updateFromHtml(childRef.current);
			setHandlers();
		}
	}, [pages]);
...
var HTMLFlipBook = import_react.memo(HTMLFlipBookForward);
//#endregion
